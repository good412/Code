function [S_final, H_final, nIter_final, objhistory_final] = GNMF_S(X, k, W, options, S, H)
% Graph regularized Symmetric Non-negative Matrix Factorization (GNMF)
%
% where
%   X
% Notation:
...
switch lower(options.Optimization)
    case {lower('Multiplicative')} 
        [S_final, H_final, nIter_final, objhistory_final] = GNMF_S_Multi(X, k, W, options, S, H);
    otherwise
