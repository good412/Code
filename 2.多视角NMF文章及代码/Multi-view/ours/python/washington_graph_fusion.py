    ...
    def gibbs(self, vectexS, graphs, fn_fusion_result, kNN, retri_amount, weights):
        ...
        for node in vectexS:
            ...
            for iter in range(retri_amount):
                ...
                tmp_rank = [ vertex for vertex in sorted(weight, key=weight.get, reverse=True)]
                Z = 0
                for t in range(len(tmp_rank)):
                    Z += self.decay(t+1, lam)
                r = random.random()
                for t in range(len(tmp_rank)):
                    value = self.decay(t+1, lam)
                    if r < value/Z : 
                        rank.append(tmp_rank[t])
                        break
                    r -= value/Z
            for img_id in rank:
...
if __name__ == "__main__":
    ...
    fn_fusion_result = 'data/washington_graph_fusion_results.txt'
    fn_label = 'data/washington_list_data_labels.txt'
    
    graphfusion = TTNG_MFR()
    
    fn_results = []
    fn_results.append('data/washington_rank_cites.txt')
    fn_results.append('data/washington_rank_content.txt')
    
    fn_result_rerankings = []
    fn_result_rerankings.append('data/washington_rerank_cites.txt')
    fn_result_rerankings.append('data/washington_rerank_content.txt')
