...
class TTNG_MFR:
    ...
    def gibbs(self, vectexS, graphs, fn_fusion_result, kNN, retri_amount, weights):
        ...
        for node in vectexS:
            rank = []
            weight = weights[node]
            for vertex in sorted(weight, key=weight.get, reverse=True):
                rank.append(vertex)
            rank = rank[0:retri_amount]
            for iter in range(iter_num):
                for i in range(1, retri_amount):
                    tmp_weight = {}
                    for j in range(0, retri_amount):
                        if i == j: continue
                        for v in graphs[rank[j]]:
                            if v not in tmp_weight:
                                tmp_weight[v] = 0
                            tmp_weight[v] += weights[rank[j]][v]
                            if rank[j] in graphs[v]:
                                tmp_weight[v] += weights[v][rank[j]]
                    Z = 0
                    for v in tmp_weight.keys():
                        Z += math.exp(tmp_weight[v]);
                    r = random.random()
                    for v in tmp_weight.keys():
                        value = math.exp(tmp_weight[v])
                        if r < value/Z : 
                            rank[i] = v
                            break
                        r -= value
...
if __name__ == "__main__":
    ...
    fn_fusion_result = 'data/3sources_graph_fusion_results.txt'
    fn_label = 'data/3sources_list_data_labels.txt'
    
    graphfusion = TTNG_MFR()
    
    fn_results = []
    fn_results.append('data/3sources_rank_bbc.txt')
    fn_results.append('data/3sources_rank_guardian.txt')
    fn_results.append('data/3sources_rank_reuters.txt')
    
    fn_result_rerankings = []
    fn_result_rerankings.append('data/3sources_rerank_bbc.txt')
    fn_result_rerankings.append('data/3sources_rerank_guardian.txt')
    fn_result_rerankings.append('data/3sources_rerank_reuters.txt')
    ...
    for i in range(num_ranks):
        ...
        search_region = 10
        lam = 1
        graph_lists.append(build_cknn_graphs.BuildKNNGraphs(fn_result, fn_result_reranking, fn_label, kNN, 150, search_region, lam))
    ...
    evaluate.Evaluate(fn_label, fn_fusion_result, retri_amount)
